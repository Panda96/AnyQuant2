...
import tool.constant.StockConstant;
...
public class StockPriceInfo {
    ...
    private void caculateAccuPrice() {
        ...
        //计算上午的累计价格
        List<String> timesBeforeNoon = DateCount.getSeconds(time1, time2, 1);
        ...
        //计算下午的累计价格
        List<String> timesAfterNoon = DateCount.getSeconds(time3, time4, 1);
        for (String time : timesAfterNoon) {
            if (timeGotInfo(time)) {
                //数据累计
                accumulateAmount += this.timeAndAmount.get(time);
                accumulateVolume += this.timeAndVolume.get(time) * 100;
